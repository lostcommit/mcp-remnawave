...
const snapshot = JSON.parse(await readFile(new URL('../openapi/remnawave-v3.4.3.openapi.json', import.meta.url)));
const manifest = JSON.parse(await readFile(new URL('../openapi/remnawave-v3.4.3.operations.json', import.meta.url)));
    ...
    return Object.entries(document.paths)
        .flatMap(([path, pathItem]) => Object.entries(pathItem)
            ...
            .map(([method, operation]) => ({
                ...
                path,
                hasJsonBody: Object.keys(operation.requestBody?.content ?? {})
                    .some((contentType) => contentType === 'application/json' || contentType.endsWith('+json')),
            })))
...
test('the generated v3 manifest covers every operation in the pinned OpenAPI snapshot', () => {
    ...
    assert.deepEqual(actual, expected.map(({ hasJsonBody: _hasJsonBody, ...operation }) => operation));
    assert.equal(new Set(manifest.operations.map(({ toolName }) => toolName)).size, expected.length);
    assert.equal(
...
test('the generated manifest has stable OpenAPI provenance and canonical tool names', () => {
    ...
    assert.equal(manifest.sourceSha256, 'ed9ca9ea55da2b9da266db8f9e1e546e9caf2831a760655a18c1ddf35218c44b');
    for (const operation of manifest.operations) {
        assert.match(operation.toolName, /^rw_v3_[a-z0-9_]+$/);
        assert.ok(operation.responses.length > 0, `${operation.operationId} has no success response`);
        assert.ok(
...
test('v3 is the default and registers every generated operation as an MCP tool', { timeout: 10_000 }, async (t) => {
...
test('v3 readonly mode exposes only documented GET operations', { timeout: 10_000 }, async (t) => {
    ...
    assert.deepEqual(
        toolNames,
        manifest.operations.filter(({ method }) => method === 'GET').map(({ toolName }) => toolName).sort(),
    );
});
